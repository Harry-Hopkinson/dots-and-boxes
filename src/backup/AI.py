...
def longest_chain_from(game, visited, i, j, length):
    ...
    # find openings
    horizontal, vertical = (
        game.board[i - 1, j] and game.board[i + 1, j],
        game.board[i, j - 1] and game.board[i, j + 1],
    )

    # follow each opening and return length
    if horizontal:
        return max(
            longest_chain_from(game, visited, i, j - 2, length + 1),
            longest_chain_from(game, visited, i, j + 2, length + 1),
        )
    elif vertical:
        return max(
...
def find_longest_chain(game):
    ...
    for i in range(1, game.r, 2):
        for j in range(1, game.c, 2):
            ...
            longest_chain = max(
                longest_chain, longest_chain_from(game, dict(), i, j, 0)
            )
