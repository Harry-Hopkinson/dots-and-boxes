...
class DotsAndBoxes:
    ...
    def mark_box(self):
        ...
        for box in boxes:
            if list(box) not in self.already_marked_boxes and list(box) != []:
                ...
                colour = player1_colour_light
                self.shade_box(box, colour)

        boxes = np.argwhere(self.board_status == 4)
        for box in boxes:
            if list(box) not in self.already_marked_boxes and list(box) != []:
                self.already_marked_boxes.append(list(box))
                colour = player2_colour_light
                self.shade_box(box, colour)
    ...
    def make_edge(self, type, logical_position):
        ...
        self.canvas.create_line(start_x, start_y, end_x,
                                end_y, fill=colour, width=edge_width)
    ...
    def display_gameover(self):
        ...
        self.canvas.create_text(
            size_of_board / 2, size_of_board / 3, font="cmr 60 bold", fill=colour, text=text)

        score_text = 'Scores \n'
        self.canvas.create_text(size_of_board / 2, 5 * size_of_board / 8, font="cmr 40 bold", fill=Green_colour,
                                text=score_text)

        score_text = 'Player 1 : ' + str(player1_score) + '\n'
        score_text += 'Player 2 : ' + str(player2_score) + '\n'
        self.canvas.create_text(size_of_board / 2, 3 * size_of_board / 4, font="cmr 30 bold", fill=Green_colour,
                                text=score_text)
        ...
        for i in range(number_of_dots):
            for j in range(number_of_dots):
                ...
                self.canvas.create_oval(start_x-dot_width/2, end_x-dot_width/2, start_x+dot_width/2,
                                        end_x+dot_width/2, fill=dot_colour,
                                        outline=dot_colour)
    ...
    def display_turn_text(self):
        ...
        self.turntext_handle = self.canvas.create_text(size_of_board - 5*len(text),
                                                       ...
                                                       font="cmr 15 bold", text=text, fill=colour)

    def shade_box(self, box, colour):
        start_x = distance_between_dots / 2 + \
        ...
        self.canvas.create_rectangle(
            start_x, start_y, end_x, end_y, fill=colour, outline='')
